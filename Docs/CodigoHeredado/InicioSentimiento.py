...
def analizar_sentimiento_multitexto(textos: list) -> list:
    ...
    return {"resultados_individuales": resultados, "estadisticas": estadisticas}
...
print(
    f"   Polaridad promedio: {resultado_multiple['estadisticas']['polaridad_promedio']:.2f}"
)
...
for i, res in enumerate(resultado_multiple["resultados_individuales"]):
    ...
    print(
        f"   → Sentimiento: {res.get('sentimiento')} (polaridad: {res.get('polaridad', 'N/A')})"
    )
    if res.get("emociones"):
        emocion_principal = (
            max(res["emociones"].items(), key=lambda x: x[1])
            if res["emociones"]
            else ("ninguna", 0)
        )
